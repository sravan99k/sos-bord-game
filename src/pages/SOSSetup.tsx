...
export default function SOSSetup() {
  ...
  const mode = ((location.state as any)?.mode as 'ai' | 'friends' | undefined) ?? 'ai';
  ...
  const [players, setPlayers] = useState<Player[]>(() => {
    if (mode === 'friends') {
      return [
        ...
        { name: "Friend", color: "hsl(0 75% 55%)", colorClass: PLAYER_COLORS[0]!.class, isAI: false },
      ];
    }
    return [
      { name: "You", color: "hsl(210 80% 30%)", colorClass: PLAYER_COLORS[1]!.class, isAI: false },
  ...
  const hasAI = useMemo(() => players.some(p => p.isAI), [players]);
    ...
    <div className="min-h-screen bg-[#FDFCF8] relative font-body p-4 md:p-8 flex flex-col">
      ...
      <div className="relative z-10 w-full max-w-4xl mx-auto flex-1 flex flex-col min-h-0">
        ...
        <div className="flex-1 overflow-y-auto pr-2 space-y-6 scrollbar-hide py-2">
          {/* Main Config Grid */}
          <div className={`grid gap-6 ${hasAI ? 'md:grid-cols-2' : 'md:grid-cols-1'}`}>
            {/* Battlefield Size */}
            <section className={`p-6 border-4 border-foreground bg-white shadow-[8px_8px_0px_rgba(0,0,0,0.1)] h-full ${hasAI ? '' : 'md:col-span-2'}`}>
                  ...
                  <button key={num} onClick={() => {
                    ...
                    if (num > players.length) {
                      for (let i = players.length; i < num; i++) {
                        const defaultIsAI = mode === 'ai' ? i === 1 : false;
                        newPlayers.push({ name: defaultIsAI ? "Robot" : (i === 1 ? "Friend" : `Player ${i + 1}`), color: PLAYER_COLORS[i % PLAYER_COLORS.length]!.value, colorClass: PLAYER_COLORS[i % PLAYER_COLORS.length]!.class, isAI: defaultIsAI });
                      }
                    } else {
