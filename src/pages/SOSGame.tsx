...
export default function SOSGame() {
  ...
  const lineCoords = useMemo<LineCoords[]>(() => {
    const el = gridContainerRef.current;
    if (!el) return [];
    const gridRect = el.getBoundingClientRect();

    return sosLines.map((line) => {
      const [sr, sc] = line.cells[0]!;
      const [er, ec] = line.cells[2]!;
      const startEl = cellRefs.current[sr]?.[sc];
      const endEl = cellRefs.current[er]?.[ec];

      if (!startEl || !endEl) return null;

      const sRect = startEl.getBoundingClientRect();
      const eRect = endEl.getBoundingClientRect();
      const x1 = (sRect.left + sRect.right) / 2 - gridRect.left;
      const y1 = (sRect.top + sRect.bottom) / 2 - gridRect.top;
      const x2 = (eRect.left + eRect.right) / 2 - gridRect.left;
      const y2 = (eRect.top + eRect.bottom) / 2 - gridRect.top;

      const len = Math.hypot(x2 - x1, y2 - y1);

      if (!Number.isFinite(len) || len <= 0) return null;

      return { x1, y1, x2, y2, len, playerIndex: line.playerIndex };
    }).filter((x): x is LineCoords => x !== null);
  }, [overlaySize.width, overlaySize.height, sosLines]);
        ...
        <main className="flex-1 flex flex-col items-center justify-center min-h-0">
          ...
          <div className="flex flex-col lg:flex-row items-center justify-center gap-8 w-full max-w-full min-h-0">
            ...
            >
              ...
              <div ref={gridContainerRef} className="relative w-full h-full grid" style={{ gridTemplateColumns: `repeat(${gridSize}, 1fr)`, gridTemplateRows: `repeat(${gridSize}, 1fr)` }}>
                {Array.from({ length: gridSize * gridSize }).map((_, i) => {
                  ...
                  return (
                    <button
                      ...
                      key={`${r}-${c}`}
                      disabled={cell.value !== null || gameOver || (players[currentPlayer]?.isAI)}
                      onClick={() => handleCellClick(r, c)}
                      className={`relative border border-foreground/10 flex items-center justify-center transition-all group p-0 overflow-hidden ${!cell.value && !gameOver ? 'hover:bg-primary/5' : ''}`}
                    >
                      {cell.value && (
                ...
                <svg className="absolute inset-0 pointer-events-none w-full h-full z-20 overflow-visible" viewBox={`0 0 ${overlaySize.width} ${overlaySize.height}`} preserveAspectRatio="none">
                  {lineCoords.map((line, idx) => {
                    const color = players[line.playerIndex]?.color || 'currentColor';
                    return (
                      <g key={idx}>
                        <line x1={line.x1} y1={line.y1} x2={line.x2} y2={line.y2} stroke={color} strokeWidth="6" strokeLinecap="round" className="opacity-15 blur-sm" />
                        <line x1={line.x1} y1={line.y1} x2={line.x2} y2={line.y2} stroke={color} strokeWidth="3" strokeLinecap="round" style={{ strokeDasharray: `${line.len}`, strokeDashoffset: `${line.len}`, animation: 'draw 0.4s ease-out forwards' }} />
                      </g>
                    );
                  })}
                </svg>
